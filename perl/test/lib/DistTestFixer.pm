...
sub fix {
    ...
    if ( $^O eq 'MSWin32' ) {
        my $bin = File::Spec->catfile($inc, "yamltest-cpan");
        my $cmd = File::Spec->catfile($inc, "yamltest-cpan.cmd");
        my $ys =  File::Spec->catfile('bin', 'yamltest');
...
pure_all ::
	$(NOECHO) $(FULLPERLRUN) -p0i.bak -e "s(\$$PERL)($(MYPERL))" blib\script\yamltest
...
    }
...
pure_all ::
	$(NOECHO) $(FULLPERLRUN) -p0i -e 's(\$$PERL)($(MYPERL))' blib/script/yamltest
...
    }
